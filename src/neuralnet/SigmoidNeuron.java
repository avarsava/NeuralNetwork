 ...
 * @since 2015-11-26
 * 
 * Either a hidden or an output node, uses a sigmoid function to return its
 * value.
 */
public class SigmoidNeuron implements CalcNeuron {
     ...
     * inputs.
     */
    public SigmoidNeuron(int n, Weight[][] newWAM) {
        this.outputs = null;
        id = n;
        inputWAM = newWAM;
    }
     ...
     * @param newWAM Matrix which describes weights between this neuron and its
     * inputs
     * @param newWAM2 Matrix which describes weights between this neuron and 
     * its outputs.
     */
    public SigmoidNeuron(int n, Weight[][] newWAM, Weight[][] newWAM2){
        this.outputs = null;
        id = n;
        inputWAM = newWAM;
        outputWAM = newWAM2;
    ...
    public static void initNeurons(Neuron[] neurons, Weight[][] weights) {
        for (int i = 0; i < neurons.length; i++) {
            neurons[i] = new SigmoidNeuron(i, weights);
        }
    }
    ...
    public static void initNeurons(Neuron[] neurons, Weight[][] weights, 
            Weight[][] weights2) {
        for (int i = 0; i < neurons.length; i++) {
            neurons[i] = new SigmoidNeuron(i, weights, weights2);
        }
    }
}
